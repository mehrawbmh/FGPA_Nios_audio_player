#include "globals.h"

/* globals used for audio record/playback */
extern volatile int buf_index_record, buf_index_play;
unsigned int l_buf[BUF_SIZE];			  // audio buffer
unsigned int r_buf[BUF_SIZE];			  // audio buffer
extern unsigned int l_buf_echo[BUF_SIZE]; // echo buffer
extern unsigned int r_buf_echo[BUF_SIZE];
extern int is_echo;
/***************************************************************************************
 * Audio - Interrupt Service Routine
 *
 * This interrupt service routine records or plays back audio, depending on which type
 * interrupt (read or write) is pending in the audio device.
 ****************************************************************************************/
void audio_ISR(struct alt_up_dev *up_dev, unsigned int id)
{
	int num_read;
	int num_written;

	unsigned int fifospace;

	if (alt_up_audio_read_interrupt_pending(up_dev->audio_dev)) // check for read interrupt
	{
		alt_up_parallel_port_write_data(up_dev->green_LEDs_dev, 0x1); // set LEDG[0] on

		// store data until the buffer is full
		if (buf_index_record < BUF_SIZE)
		{
			num_read = alt_up_audio_record_r(up_dev->audio_dev, &(r_buf[buf_index_record]),
											 BUF_SIZE - buf_index_record);
			/* assume we can read same # words from the left and right */
			(void)alt_up_audio_record_l(up_dev->audio_dev, &(l_buf[buf_index_record]),
										num_read);
			buf_index_record += num_read;

			if (buf_index_record == BUF_SIZE)
			{
				// done recording
				printf("BUf index done recording\n");
				alt_up_parallel_port_write_data(up_dev->green_LEDs_dev, 0); // turn off LEDG
				alt_up_audio_disable_read_interrupt(up_dev->audio_dev);
			}
		}
	}

	if (alt_up_audio_write_interrupt_pending(up_dev->audio_dev)) // check for write interrupt to play
	{
		if(is_echo == 1)
			alt_up_parallel_port_write_data(up_dev->green_LEDs_dev, 0x8); // set LEDG[4] on
		else 
			alt_up_parallel_port_write_data(up_dev->green_LEDs_dev, 0x2); // set LEDG[1] on
		// output data until the buffer is empty
		if (buf_index_play < BUF_SIZE)
		{
			if (is_echo == 1)
			{
				num_written = alt_up_audio_play_r(up_dev->audio_dev, &(r_buf_echo[buf_index_play]), BUF_SIZE - buf_index_play);
				/* assume that we can write the same # words to the left and right */
				(void)alt_up_audio_play_l(up_dev->audio_dev, &(l_buf_echo[buf_index_play]), num_written);
			}
			else
			{
				num_written = alt_up_audio_play_r(up_dev->audio_dev, &(r_buf[buf_index_play]), BUF_SIZE - buf_index_play);
				/* assume that we can write the same # words to the left and right */
				(void)alt_up_audio_play_l(up_dev->audio_dev, &(l_buf[buf_index_play]), num_written);
			}

			buf_index_play += num_written;

			if (buf_index_play == BUF_SIZE)
			{
				// done playback
				alt_up_parallel_port_write_data(up_dev->green_LEDs_dev, 0); // turn off LEDG
				alt_up_audio_disable_write_interrupt(up_dev->audio_dev);
			}
		}
	}

	// if (alt_up_audio_write_interrupt_pending(up_dev->audio_dev))	// check for write interrupt to echo
	// {
	// 	alt_up_parallel_port_write_data (up_dev->green_LEDs_dev, 0x5); // set LEDG[4] on

	// 	int i;
	// 	const int sampleP1 = 25000;
	// 	const int sampleP2 = 15000;
	// 	for (i = sampleP1; i < BUF_SIZE; i++)
	// 	{
	// 		r_buf_echo[i] = (r_buf[i] + r_buf[i - sampleP2] + r_buf[i - sampleP1]);
	// 		l_buf_echo[i] = (l_buf[i] + l_buf[i - sampleP2] + l_buf[i - sampleP1]);
	// 	}
	// 	for (i = 0; i < sampleP1; i++)
	// 	{
	// 		r_buf_echo[i] = r_buf[i];
	// 		l_buf_echo[i] = l_buf[i];
	// 	}

	// 	// output data until the buffer is empty
	// 	if (buf_index_play < BUF_SIZE)
	// 	{
	// 		num_written = alt_up_audio_play_r (up_dev->audio_dev, &(r_buf_echo[buf_index_play]), BUF_SIZE - buf_index_play);
	// 		/* assume that we can write the same # words to the left and right */
	// 		(void) alt_up_audio_play_l (up_dev->audio_dev, &(l_buf_echo[buf_index_play]),
	// 			num_written);
	// 		buf_index_play += num_written;

	// 		if (buf_index_play == BUF_SIZE)
	// 		{
	// 			// done playback
	// 			alt_up_parallel_port_write_data (up_dev->green_LEDs_dev, 0); // turn off LEDG
	// 			alt_up_audio_disable_write_interrupt(up_dev->audio_dev);
	// 		}
	// 	}
	// }

	return;
}
